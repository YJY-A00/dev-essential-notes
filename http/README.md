...
# 实操5 Cookie 和 Token验证
   1. Cookie:我打开了豆包的网址，找到了Cookie
   ![](2026-08-13-19-03-05.png)
   2. 打开post.html 页面，按 F12 进入控制台（Console）。按回车（它会在请求头里手动加一个 Token）：执行后，在网络面板点开这条 posts/1 请求，在“请求标头”里找 Authorization 字段。（代码在截图）
   ![](2026-08-13-19-09-23.png)
   ![](2026-08-13-19-17-53.png)
   # Token 的核心机制：

Authorization：固定字段名，表示“我要携带身份凭证”。
Bearer：固定前缀，表示“这是一个令牌（Token）”。
mock-token-123456：你模拟的 Token 值。真实场景下，这是一串加密后的字符串（如 JWT）。